...
export function ReceiverTab({
  ...
  const handleLookupSelect = useCallback(
    (entry: LookupEntry) => {
      if (lookupType === "BANK") {
        onChange("RECEIVER_BANK_CODE", entry.CODE);
      } else if (lookupType === "DVQHNS") {
        onChange("RECEIVER_GL_SEGMENT2", entry.CODE);
      }
    },
    [lookupType, onChange],
  );
      ...
      <div className="grid grid-cols-3 gap-x-[18px] gap-y-3 max-[960px]:grid-cols-2 max-[600px]:grid-cols-1">
        ...
        <div>
          <label className={labelCls}>
            Tên người hưởng <span className="text-[#cc0000]">*</span>
          </label>
          <input
            type="text"
          ...
          {errors.RECEIVER_NAME && (
            <div className={errorCls}>{errors.RECEIVER_NAME}</div>
          )}
        </div>

        {/* Dia chi */}
        <div>
          <label className={labelCls}>Địa chỉ</label>
          <input
            type="text"
        ...
        <div>
          ...
          {errors.RECEIVER_GL_SEGMENT2 && (
            <div className={errorCls}>{errors.RECEIVER_GL_SEGMENT2}</div>
          )}
        </div>
        ...
        <div>
          ...
          {errors.RECEIVER_BANK_CODE && (
            <div className={errorCls}>{errors.RECEIVER_BANK_CODE}</div>
          )}
        </div>
        ...
        <div>
          <label className={labelCls}>
            Tên tài khoản <span className="text-[#cc0000]">*</span>
          </label>
          <input
            type="text"
          ...
          {errors.RECEIVER_ACCOUNT_NAME && (
            <div className={errorCls}>{errors.RECEIVER_ACCOUNT_NAME}</div>
          )}
        </div>
        ...
        <div>
          ...
          <input
            type="date"
            value={data.RECEIVER_ISSUED_DATE || ""}
            onChange={handleChange("RECEIVER_ISSUED_DATE")}
            className={inputCls}
            readOnly={readOnly}
          />
        </div>
