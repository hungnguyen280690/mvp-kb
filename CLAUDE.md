...
1. **Stage 1 (BA)**: **BA Agent** thẩm định yêu cầu, sinh đặc tả chi tiết, thuật ngữ và **Phân tích tác động nghiệp vụ**.
2. **Stage 2 (Design)**: **SA Agent** thiết kế API, DB Schema và **Phân tích tác động hệ thống**.
3. **Stage 3 (Dev)**: **Fullstack Dev Agent** thực thi mã nguồn, đảm bảo chất lượng FE/BE và **Phân tích tác động mã nguồn**.
4. **Stage 4 (Test)**: **QA Agent** thực hiện Automation Test (**Playwright**) và rà soát tác động hồi quy.
...
- Giai đoạn 2: `cd workspaces/sa/`
- Giai đoạn 3: `cd workspaces/dev/`
- Giai đoạn 4: `cd workspaces/qa/`

## 4. Các Quy chuẩn Bắt buộc (Mandatory Conventions)

- **BẮT BUỘC TẠO PLAN**: Mọi Agent (BA, SA, Dev, QA) phải tạo file Plan trong `gates/` (`BA-Plan`, `SA-Plan`, `Dev-Plan`, `QA-Plan`) và **chờ con người duyệt** trước khi thực hiện bất kỳ hành động nào sau đó chạy khi **con người đã duyệt**.
...
- **Luôn làm việc trên nhánh (Feature Branch)**: TUYỆT ĐỐI KHÔNG làm việc trực tiếp trên nhánh `main` và `dev`. Khi nhận tính năng mới (VD: FT-002), phải tạo nhánh `feature/FT-002` - Mẫu là feature/FT-XXX.
- **Được phép cập nhật Global Docs**: Các Agent ĐƯỢC PHÉP và BẮT BUỘC phải cập nhật các thay đổi có tính toàn cục thẳng vào thư mục `docs/`. Nhưng phải cập nhật **Cập nhật tác động**.
  - _Ví dụ_: BA phát hiện thuật ngữ mới -> Cập nhật `docs/domain/glossary.md`. SA thiết kế thêm Service mới -> Cập nhật danh sách vào `docs/ARCHITECTURE.md`.
...
_Làm sao để xác định vai trò?_

1. **Dựa vào chỉ định:** Người dùng yêu cầu đích danh (VD: "Đóng vai BA phân tích...").
2. **Dựa vào trạng thái Sign-off (Cổng kiểm soát):** Hãy dùng lệnh đọc thư mục `gates/` để xem tính năng hiện tại đã được ký duyệt tới đâu:
...
- **2026-05-19** | **System** | Tích hợp Phân tích tác động (Impact Analysis) vào cả 4 giai đoạn. Thiết lập Playwright cho QA. Bổ sung cơ chế Rework Loop và luồng Hotfix.
