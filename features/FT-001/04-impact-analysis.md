...
### 3.1 Backend — `backend/ltt-core/`

**Domain Layer** (`src/main/java/com/kb/ltt/domain/`):
| File | Mô tả |
|------|-------|
| `model/PayOrder.java` | Domain aggregate: state machine, SoD validation, 7 trạng thái |
| `model/PayOrderLine.java` | Value object: CCID 12 segments, amount |
| `model/PayOrderAttachment.java` | Value object: file metadata |
| `model/PayOrderStatus.java` | Enum 7 trạng thái |
| `model/ChannelCode.java` | Enum LNH, TTSP, LIEN_KHO_BAC |
| `model/LnhTransactionType.java` | Enum loại giao dịch LNH |
| `model/ExpenseType.java` | Enum loại chi |
| `exception/BusinessException.java` | Base exception |
| `exception/InvalidStatusTransitionException.java` | State machine violation |
| `exception/OptimisticLockException.java` | ETag/version conflict |
| `exception/SoDViolationException.java` | Segregation of Duties violation |

**Application Layer** (`src/main/java/com/kb/ltt/application/`):
| File | Mô tả |
|------|-------|
| `usecase/CreatePayOrderUseCase.java` | BDD-01 TC.1.01 |
| `usecase/GetPayOrderUseCase.java` | Detail + ETag |
| `usecase/UpdatePayOrderUseCase.java` | BDD-02 |
| `usecase/DeletePayOrderUseCase.java` | BDD-05 soft-delete |
| `usecase/ListPayOrderUseCase.java` | BDD-04 filter+paginate |
| `usecase/CopyPayOrderUseCase.java` | BDD-07 |
| `usecase/SubmitPayOrderUseCase.java` | BDD-01 TC.1.12, VAL-01..19 |
| `usecase/CheckApprovePayOrderUseCase.java` | BDD-03 TC.3.01 SoD |
| `usecase/ApprovePayOrderUseCase.java` | BDD-03 TC.3.03 3-way SoD |
| `usecase/ReturnPayOrderUseCase.java` | BDD-03 TC.3.05 |
| `usecase/RejectPayOrderUseCase.java` | BDD-03 TC.3.06 |
| `usecase/ExportPayOrderUseCase.java` | BDD-06 Excel/CSV/PDF |
| `usecase/AttachmentUseCase.java` | BDD-01 TC.1.06 upload/download |
| `usecase/ValidateCcidUseCase.java` | Mock valid=true |
| `usecase/LookupUseCase.java` | Hard-coded LOV |
| `usecase/AuditLogQueryUseCase.java` | Paged audit log |
| `usecase/ApprovalStatusUseCase.java` | Stepper response |
| `dto/` (11 DTO files) | Request/Response DTOs |
| `model/UserContext.java` | Current user context |

**Infrastructure Layer** (`src/main/java/com/kb/ltt/infrastructure/`):
| File | Mô tả |
|------|-------|
| `persistence/entity/` (7 files) | JPA Entities: PayOrder, Line, Attachment, Approval, AuditLog, Idempotency, RefNoSequence |
| `persistence/repository/` (7 files) | Spring Data JPA repositories |
| `persistence/mapper/PayOrderMapper.java` | MapStruct Entity↔Domain↔DTO |
| `persistence/specification/PayOrderSpecification.java` | JPA Specification cho filter |
| `service/RefNoGeneratorService.java` | Atomic REF_NO sequence |
| `service/AuditHashChainService.java` | SHA-256 hash chain |
| `service/IdempotencyService.java` | X-Idempotency-Key TTL 24h |
| `service/IdempotencyResult.java` | Result wrapper |
| `util/HashUtil.java` | SHA-256 utility |
| `security/JwtAuthFilter.java` | JWT filter + dev bypass |
| `security/SecurityConfig.java` | Spring Security config |
| `web/PayOutManualController.java` | 20 REST endpoints |
| `web/GlobalExceptionHandler.java` | Map exception → ErrorResponse |
| `web/IdempotencyInterceptor.java` | X-Idempotency-Key validation |
| `web/WebMvcConfig.java` | MVC config + interceptors |
| `config/JacksonConfig.java` | JSON serialization config |
| `config/OpenApiConfig.java` | Swagger/OpenAPI config |

### 3.2 Frontend — `frontend/apps/ltt-ui/`

**Shared Components** (`src/components/`):
| File | Mô tả |
|------|-------|
| `StatusBadge.tsx` | CVA badge 7 trạng thái |
| `LoadingSpinner.tsx` | SVG spinner (sm/md/lg) |
| `ErrorBoundary.tsx` | React class error boundary |
| `ConfirmDialog.tsx` | Radix Dialog xác nhận |
| `FormField.tsx` | Label + input wrapper + error |
| `MoneyInput.tsx` | VND format number input |
| `DatePickerField.tsx` | Native date input |
| `DataTable.tsx` | Sort + paginate generic table |
| `LovSelect.tsx` | Lookup popup (Radix Dialog + search) |
| `ExportDialog.tsx` | Xuất dữ liệu Excel/CSV/PDF |
| `CopyDialog.tsx` | Sao chép lệnh + chọn ngày |
| `DeleteDialog.tsx` | Xóa + lý do + checkbox xác nhận |
| `AttachmentPanel.tsx` | Upload/list/download/delete file |
| `ApprovalStepperPanel.tsx` | Maker→Checker→Approver stepper |
| `AuditLogPanel.tsx` | Paginated audit log table |

**Pages** (`src/pages/`):
| File | Mô tả |
|------|-------|
| `PayOutManualListPage.tsx` | Danh sách + filter + sort + export |
| `PayOutManualFormPage.tsx` | Form 4-tab tạo/sửa (zod + RHF) |
| `PayOutManualDetailPage.tsx` | Chi tiết + action buttons RBAC |

**API Layer** (`src/api/`):
| File | Mô tả |
|------|-------|
| `client.ts` | Axios instance + JWT header |
| `pay-out-manual.ts` | 20 API call functions |
| `hooks.ts` | React Query hooks |

**Types & Utils** (`src/`):
| File | Mô tả |
|------|-------|
| `types/pay-order.ts` | TypeScript types toàn bộ domain |
| `lib/utils.ts` | cn(), formatCurrency(), formatDate() |
| `contexts/AuthContext.tsx` | JWT dev mock context |
| `bootstrap.tsx` | Router + QueryClient setup |

### 3.3 Backend Tests

| Test Class                                           | Coverage                                 |
| ---------------------------------------------------- | ---------------------------------------- |
| `domain/PayOrderStateMachineTest.java`               | 32 tests: valid/invalid transitions, SoD |
| `domain/PayOrderLineTest.java`                       | 4 tests: line validation                 |
| `application/usecase/*Test.java`                     | 57 tests: all use cases                  |
| `infrastructure/service/*Test.java`                  | RefNo, AuditHashChain, Idempotency       |
| `infrastructure/web/PayOutManualControllerTest.java` | REST layer tests                         |

**Tổng: 97 tests, 0 failures, 0 errors**
...
- **2026-05-20** | **Fullstack Dev Agent** | FT-001 | Hoàn thành Section 3 — Code Impact (60+ Java files, 18 TSX components/pages, 97 unit tests pass).
