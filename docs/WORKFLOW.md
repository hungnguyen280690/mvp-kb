...
```text
features/FT-001/
├── 01-po-requirement.md     (Đầu vào từ PO)
...
├── *.html                   (Đầu vào bắt buộc — Tối thiểu 1 file HTML mẫu giao diện)
├── *.css                    (Đầu vào bắt buộc — File CSS mẫu giao diện)
...
- **Đầu vào bắt buộc (từ PO/Con người)**:
  1. Yêu cầu thô từ Product Owner (`01-po-requirement.md`).
  2. **Tối thiểu 1 file HTML mẫu giao diện** (`.html`) — Màn hình mẫu để BA phân tích layout, trường dữ liệu, nút bấm.
  3. **File CSS mẫu** (`.css`) — Quy tắc style liên quan đến giao diện mẫu.
  4. **File Use Case** (`.md`) — Mô tả use case nghiệp vụ (tùy chọn nếu PO yêu cầu thô đã chi tiết).
